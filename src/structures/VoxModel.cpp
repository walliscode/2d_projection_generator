...
// Helper to get sf::Color from palette and color index
static sf::Color decodeColor(uint8_t colorIndex, const uint32_t palette[256]) {
  int idx = colorIndex;
  if (idx == 0)
    idx = 1; // Safety: treat 0 as 1
  uint32_t color = palette[idx - 1];
  return sf::Color(color & 0xFF, (color >> 8) & 0xFF, (color >> 16) & 0xFF,
                   (color >> 24) & 0xFF);
}
...
bool loadVox(const std::string &filename, VoxModel &model) {
  ...
  // Init palette
  std::memcpy(model.palette, default_palette, sizeof(default_palette));

  // Find MAIN chunk
  char chunkId[4];
  while (f.read(chunkId, 4)) {
    uint32_t chunkSize = read_u32(f);
    uint32_t childChunks = read_u32(f);
    if (std::strncmp(chunkId, "MAIN", 4) == 0)
      break;
    f.seekg(chunkSize + childChunks, std::ios::cur);
  }
  // Now MAIN: read all children
  while (f.read(chunkId, 4)) {
    uint32_t chunkSize = read_u32(f);
    uint32_t childChunks = read_u32(f);
    std::streampos nextChunk = f.tellg();
    nextChunk += chunkSize;

    if (std::strncmp(chunkId, "SIZE", 4) == 0) {
      model.sizeX = read_u32(f);
      model.sizeY = read_u32(f);
      model.sizeZ = read_u32(f);
    } else if (std::strncmp(chunkId, "XYZI", 4) == 0) {
      uint32_t numVoxels = read_u32(f);
      model.voxels.resize(numVoxels);
      for (uint32_t i = 0; i < numVoxels; ++i) {
        model.voxels[i].x = read_u8(f);
        model.voxels[i].y = read_u8(f);
        model.voxels[i].z = read_u8(f);
        model.voxels[i].colorIndex = read_u8(f);
        model.voxels[i].color =
            decodeColor(model.voxels[i].colorIndex, model.palette);
      }
    } else if (std::strncmp(chunkId, "RGBA", 4) == 0) {
      for (int i = 0; i < 256; ++i) {
        uint8_t r = read_u8(f), g = read_u8(f), b = read_u8(f), a = read_u8(f);
        model.palette[i] = (a << 24) | (b << 16) | (g << 8) | r;
      }
    }
    f.seekg(nextChunk);
  }
  return !model.voxels.empty();
}
